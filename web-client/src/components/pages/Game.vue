...
const undo = () => {
    store.dispatch('undo', new UndoRequest(store.getters['getCurrentGame'].id,
        store.getters['getCurrentGame'].board.tiles))
}

const nextBestMove = () => {
    store.dispatch('nextBestMove', new SolveRequest(store.getters['getCurrentGame'].board.tiles));
}
                ...
                <div class="flex items-center grow w-full max-w-md">
                    <Card class="w-full">
                        <div class="grid grid-cols-2 content-center">
                            ...
                            <div class="flex flex-row lg:mx-7 justify-between mx-3">
                                <ArrowPathIcon @click="reset" class="w-7 my-auto sm:mx-3"/>
                                <ArrowUturnLeftIcon @click="undo" class="w-7 my-auto sm:mx-3"/>
                                <LightBulbIcon @click="nextBestMove" class="w-7 my-auto sm:mx-3"/>
                            </div>
                        </div>
                    </Card>
                </div>
